...
function UrgentNotImportant() {
  const {t} = useTranslation();
  const location = useLocation();
  const navigate = useNavigate();
  const [tasks, setTasks] = useState([]);
  const quadrantType = 'urgent-not-important'; 
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-pink-50 via-blue-50 to-green-50">
      ...
      <div className="p-4 flex items-center bg-white shadow-sm">
        ...
        <h1 className="text-2xl font-bold text-gray-800">{t('urgent_not_important.title')}</h1>
      </div>
      ...
      <div className="flex-1 px-4 py-6 space-y-4 overflow-auto">
        {tasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
            ...
            <p className="text-lg">{t('urgent_not_important.no_tasks')}</p>
            <p className="text-sm">{t('urgent_not_important.add_task')}</p>
          </div>
        ) : (
          tasks.map((task) => (
            <div 
            ...
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
              ...
              >
                <div className="flex items-center">
                  <span className="text-xs mr-1">{t('urgent_not_important.schedule')}</span>
